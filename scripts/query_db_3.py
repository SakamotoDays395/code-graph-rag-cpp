...
# Tree-sitter might extract 'class' or 'interface'
cursor.execute('''
    SELECT e.name, e.file_path, COUNT(r.id) as method_count
    FROM entities e
    JOIN relationships r ON e.id = r.from_id
    WHERE e.type IN ('class', 'interface', 'struct') AND e.name LIKE '%Analyzer' AND r.type = 'contains'
    GROUP BY e.name
    ORDER BY method_count DESC
    LIMIT 5
''')
for row in cursor.fetchall():
    print(f"{row['name']} in {row['file_path']}: {row['method_count']} contained elements")
