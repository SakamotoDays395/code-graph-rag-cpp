...
public:
    ...
    // fmt format-string overloads (for "msg {}", args...)
    ...
    // Plain string_view overloads (for "msg" with no format args)
...
template <typename... Args>
inline void log_debug(std::string_view c, fmt::format_string<Args...> f, Args&&... args) {
    Logger::global().debug(c, f, std::forward<Args>(args)...);
}
inline void log_debug(std::string_view c, std::string_view m) { Logger::global().debug(c, m); }

template <typename... Args>
inline void log_info(std::string_view c, fmt::format_string<Args...> f, Args&&... args) {
    Logger::global().info(c, f, std::forward<Args>(args)...);
}
inline void log_info(std::string_view c, std::string_view m) { Logger::global().info(c, m); }

template <typename... Args>
inline void log_warn(std::string_view c, fmt::format_string<Args...> f, Args&&... args) {
    Logger::global().warn(c, f, std::forward<Args>(args)...);
}
inline void log_warn(std::string_view c, std::string_view m) { Logger::global().warn(c, m); }

template <typename... Args>
inline void log_error(std::string_view c, fmt::format_string<Args...> f, Args&&... args) {
    Logger::global().error(c, f, std::forward<Args>(args)...);
}
